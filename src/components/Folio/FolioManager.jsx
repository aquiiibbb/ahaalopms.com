...
import { getSequenceConfig, formatSequence, getHotelProfile, getHotelTerms, getCancellationPolicies, getHotelAddons, getBusinessDate, getTaxRules, getTaxInclusiveSetting, getRatePlans, getBusinessSources } from '../../services/hotelConfig';
...
export const FolioManager = ({
  ...
  const handleSaveNotesList = async (updatedList, auditAction = "Note Updated", auditDetails = "Updated reservation notes") => {
    ...
    try {
      const pmsB = JSON.parse(localStorage.getItem('pms_bookings') || '[]');
      ...
      if (idx !== -1) {
        ...
        localStorage.setItem('pms_bookings', JSON.stringify(pmsB));
      }
    } catch (e) {}
  ...
  const getGuestScannedImages = (res) => {
    ...
    if (Array.isArray(res.scannedImages) && res.scannedImages.length > 0) {
      ...
      if (res.idFront || res.scannedIdUrl || res.idDocumentUrl) {
        list.push({
          id: 'scanned_id_front',
          title: `${res.idType || "US Driver's License"} - Front`,
          idType: res.idType || "US Driver's License",
          idNumber: res.idNumber || 'Verified',
          issueCountry: res.idCountry || 'United States',
          url: res.idFront || res.scannedIdUrl || res.idDocumentUrl,
      ...
      if (res.idBack) {
        list.push({
          id: 'scanned_id_back',
          title: `${res.idType || "US Driver's License"} - Back (Barcode)`,
          idType: res.idType || "US Driver's License",
          idNumber: res.idNumber || 'Verified',
          issueCountry: res.idCountry || 'United States',
          url: res.idBack,
  ...
  const getGuestCapturedImages = (res) => {
    ...
    if (Array.isArray(res.capturedImages) && res.capturedImages.length > 0) {
      list = [...res.capturedImages];
    } else {
      if (res.guestPhoto || res.photoUrl || res.capturedPhoto) {
        list.push({
          id: 'captured_guest_photo',
          title: 'Guest Check-In Photo',
          url: res.guestPhoto || res.photoUrl || res.capturedPhoto,
  ...
  const handleSaveNewScannedDoc = (e) => {
    ...
    currentRes.guestPhoto = docData.url || currentRes.guestPhoto;
    currentRes.photoUrl = docData.url || currentRes.photoUrl;

    syncBookingUpdate(currentRes.id, {
      scannedImages: updatedList,
      guestPhoto: docData.url || currentRes.guestPhoto,
      photoUrl: docData.url || currentRes.photoUrl
    });
  ...
  const handleDeleteScannedDoc = (docId) => {
    ...
    syncBookingUpdate(currentRes.id, { scannedImages: updatedList });
  ...
  const handleDeleteCapturedPhoto = (photoId) => {
    ...
    syncBookingUpdate(currentRes.id, { capturedImages: updatedList });
  ...
  const handleAddPayment = async (e) => {
    ...
    if (selectedMethod === "saved_card") {
      ...
      modeType = "Online";
    } else if (selectedMethod === "security_deposit" || selectedMethod.toLowerCase().includes("security deposit")) {
      methodLabel = "Security Deposit (Cash)";
      modeType = "Cash";
  ...
  const handleDeleteChargeItem = async (itemId, tabName) => {
    ...
    try {
      const { updateBooking: apiUpdateBooking } = await import('../../services/api');
      if (resId) {
        await apiUpdateBooking(resId, { [tabName]: updatedTab, skipAuditLog: true });
        if (typeof updateBooking === 'function') {
          updateBooking(resId, { [tabName]: updatedTab, skipAuditLog: true });
        }
      }
      window.dispatchEvent(new CustomEvent('pms_bookings_updated'));
    } catch (err) {
  ...
  const isSecurityDepositPaymentMethod = String(currentRes?.paymentMethod || '').toLowerCase().includes('security deposit');
  ...
  const rawDepositsList = (Array.isArray(currentRes?.deposits) && currentRes.deposits.length > 0
    ...
    if (!dep) return false;
    const isFakeDeposit = !isSecurityDepositPaymentMethod &&
      (dep.note === 'Security Deposit Collected at Booking' || dep.note === 'Security Deposit Collected at Check-In') &&
      Number(dep.amount || 0) === Number(currentRes?.advanceAmount || currentRes?.totalAmount || 0);
    return !isFakeDeposit;
  });
  ...
  if (mergedDeposits.length === 0 && isSecurityDepositPaymentMethod && Number(currentRes?.depositAmount || currentRes?.depositBalance || 0) > 0 && currentRes?.depositStatus !== 'refunded' && currentRes?.depositStatus !== 'applied') {
    mergedDeposits.push({
  ...
  return (
    <div className="folio-view">
            ...
            <div className="folio-actions" style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '2px', background: '#ffffff', padding: '2px', borderRadius: '6px', border: '1px solid #cbd5e1' }}>
                <button 
                  ...
                  className="btn btn-sm" 
                  onClick={() => {
                    const nextVal = !isPrepaid;
                    setIsPrepaid(nextVal);
                    const act = 'Prepaid';
                    const det = nextVal ? 'Enabled Prepaid status (encrypt room rates on printed invoice)' : 'Disabled Prepaid status (show room rates on printed invoice)';
                    recordAuditLog(act, det);
                    const resId = currentRes?.id || selectedResId;
                    if (resId) {
                      try {
                        if (typeof updateBooking === 'function') updateBooking(resId, { isPrepaid: nextVal, auditAction: act, auditDetails: det });
                        import('../../services/api').then(({ updateBooking: apiUpdateBooking }) => {
                          if (apiUpdateBooking) apiUpdateBooking(resId, { isPrepaid: nextVal, auditAction: act, auditDetails: det });
                        });
                      } catch(e) {}
                    }
                  }} 
                ...
                <button className="btn btn-sm" onClick={() => { setShowPrintOptionsModal(true); recordAuditLog('Print Invoice', 'Opened Print Invoice options & preview'); }} style={{ border: 'none', background: 'transparent', padding: '3px 8px', fontSize: '11px', fontWeight: '700', color: '#0f172a', cursor: 'pointer' }}>
                  <Printer size={13} /> Print Invoice
                </button>
                <button className="btn btn-sm" onClick={() => { setShowEmailModal(true); recordAuditLog('Email Receipt', 'Opened Email Receipt dispatch dialog'); }} style={{ border: 'none', background: 'transparent', padding: '3px 8px', fontSize: '11px', fontWeight: '700', color: '#0f172a', cursor: 'pointer' }}>
                        ...
                        return displayPayments.map((p, pIdx) => {
                          ...
                          return (
                            <tr key={p.id || `pay_${p.date}_${pIdx}`} style={{ background: '#f0fdf4' }}>
                              ...
                              <td>
                                <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                  <span style={{ fontSize: '11.5px', color: '#059669', fontWeight: '700' }}>✓ Paid</span>
                                </div>
                              </td>
                            </tr>
                          );
      ...
      {showTransferModal && (() => {
        ...
        const list = Array.isArray(pmsReservations) && pmsReservations.length > 0 ? pmsReservations : (JSON.parse(localStorage.getItem('pms_bookings') || '[]'));
      ...
      {showModifySegmentModal && (
        <div className="modal-backdrop" onClick={() => setShowModifySegmentModal(false)} style={{ background: 'rgba(15, 23, 42, 0.65)', backdropFilter: 'blur(4px)', zIndex: 99999 }}>
      ...
      {showEditNoteModal && (
        <div className="modal-backdrop" onClick={() => setShowEditNoteModal(false)} style={{ zIndex: 999999 }}>
