...
export const MasterReport = () => {
  const pmsContext = usePMS() || {};
  const rooms = pmsContext.rooms || [];
  const reservations = pmsContext.reservations || [];
  const folios = pmsContext.folios || {};
  const auditLogs = pmsContext.auditLogs || [];
  const taxRules = pmsContext.taxRules || [];
  const businessDate = pmsContext.businessDate || new Date().toISOString().slice(0, 10);
  const showToast = pmsContext.showToast || (() => {});
